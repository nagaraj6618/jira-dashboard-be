...
async function fetchWorkflowSchemes() {
    const url = `${JIRA_SITE}/rest/api/3/workflowscheme`;    
    const all = await fetchPaginated(url, 50);
    return all;

}

async function fetchPaginated(url, maxResults = 50, valueKey = 'values') {
...
async function fetchWorkflows() {
  const all = await fetchPaginated(`${JIRA_SITE}/rest/api/3/workflow/search`, 50);
  return all;
}
async function fetchScreens() {
  return await fetchPaginated(`${JIRA_SITE}/rest/api/3/screens`, 100);
}
async function fetchScreenSchemes() {
  return await fetchPaginated(`${JIRA_SITE}/rest/api/3/screenscheme`, 50);
}
async function getCustomFields() {
  const { data } = await axios.get(`${JIRA_SITE}/rest/api/3/field`, { headers, auth });
  return data.filter(f => f.custom);
}
async function fetchNotificationSchemes() {
...
async function fetchPermissionSchemes() {
...
async function fetchPrioritySchemes() {
...
async function fetchIssueTypeScreenSchemes() {
  const all = await fetchPaginated(`${JIRA_SITE}/rest/api/3/issuetypescreenscheme`, 50);
...
async function fetchFieldConfigurations() {
  const all = await fetchPaginated(`${JIRA_SITE}/rest/api/3/fieldconfiguration`, 50);
...
async function fetchFieldConfigurationSchemes() {
  const all = await fetchPaginated(`${JIRA_SITE}/rest/api/3/fieldconfigurationscheme`, 50);
...
async function fetchIssueSecuritySchemes() {
...
async function fetchProjects() {
  const all = await fetchPaginated(`${JIRA_SITE}/rest/api/3/project/search`, 50, 'values');
  return all.map(p => ({ ID: p.id, Key: p.key, Name: p.name, "Project Type": p.projectTypeKey }));
}
async function fetchIssueLinkTypes() {
...
async function fetchFilters() {
  return await fetchPaginated(`${JIRA_SITE}/rest/api/3/filter/search`, 50);
}
async function fetchBoards() {
  return await fetchPaginated(`${JIRA_SITE}/rest/agile/1.0/board`, 50);
}
async function fetchDashboards() {
  const all = await fetchPaginated(`${JIRA_SITE}/rest/api/3/dashboard`, 50, 'dashboards');
